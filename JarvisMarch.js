...
function getCoordinates(geometry){
    ...
    if(type !== "MultiPolygon" && type !== "Polygon" && type !== "MultiLineString"){
        return coordinates.map((coord,i) => ({ type, lat: coord[1], lng: coord[0]}))
    }else if(type === "Polygon"){
        // Polygon inspection
        // return coordinates.map((coord,i) => ({ type, lat: coord[i][1], lng: coord[i][0]})) 
    }else if(type === 'MultiLineString'){
        return coordinates.map(coords => coords.map(c => ({ type, lat: c[1], lng: c[0]})))
    }else {
...
function GeoJson(){
    var bufferData = IOfileObj.readFileSync("polygonTest/test5.geojson");
    ...
    JsonData.forEach(data=>{
        // arrayData.push(data.geometry);
        // console.log(data.geometry)

        if(data.geometry.type !== "Point"){
            const geoCooordinates = getCoordinates(data.geometry)
            elementsData.push(geoCooordinates)
        }
...
function getFileData(filePath){
    ...
    arrayCoordinates = JsonData["elements"].filter(i=>i.type ===
        'node');
...
function writeResultData(ConvexHulldata, coordinatesData, fileName){
    ...
    IOfileObj.writeFileSync(fileName+"convexHull.txt",JSON.stringify(ConvexHulldata) );
    IOfileObj.writeFileSync(fileName+"coordinates.txt",JSON.stringify(coordinatesData) ); 
}
